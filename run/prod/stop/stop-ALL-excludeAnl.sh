...
if [ $# -gt 1 ]; then
  echo ""
  echo "Usage: stop-ALL-excludeAnl.sh [<SERVER_FQDN>]"
  echo "Example:"
  echo "  Default(All node): stop-ALL-excludeAnl.sh"
  echo "  Single node:       stop-ALL-excludeAnl.sh datanode01.dev.bigdata"
  echo "  Multi node:        stop-ALL-excludeAnl.sh datanode[01:05:09].dev.bigdata"
  echo "  Range node:        stop-ALL-excludeAnl.sh datanode[01-05].dev.bigdata"
  echo ""
  exit
fi
